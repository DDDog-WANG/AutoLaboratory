...
for n in tqdm(range(args.t)):
    ...
    obs, reward, done, _ = env_wrap.step(action)
    obs_wrap, reward_wrap, _, _ = env_wrap.step(action)

    obs_seq.append(obs_wrap)
    reward_seq.append(reward_wrap)
...
np.save("obs_seq.npy", obs_seq)
reward_seq = np.array(reward_seq)
np.save("reward_seq.npy", reward_seq)
